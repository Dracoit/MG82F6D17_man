...
void InitDMA()
{
	...
	DMA_SetXRAMStartaddr((u16)(&RcvBuf[0]));
	DMA_SetXRAMStartaddrRL((u16)(&RcvBuf[0]));
	DMA_SetCount(65536-(UART0_RX_BUFF_SIZE));
	DMA_SetCountRL(65536-(UART0_RX_BUFF_SIZE));
...
void main()
{
	...
	DMA_Start();		// start DMA
	RI0=0;				// clear UART0 receive flag
	...
	while(1)
	{
		...
		{// 收到一组数据
			bDMAFlag=0;
			SendStr("\nRx:");
			for(i=0;i<(UART0_RX_BUFF_SIZE);i++)
			{
				SendHex(RcvBuf[i]);
				SendByte(' ');
			}
